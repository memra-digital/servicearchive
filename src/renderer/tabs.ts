...
export const addTab = (article: Service) => {
    ...
    setTimeout(() => {
        tabbar.innerHTML += `
            <div class="tab" id="tab-${article.id}">
                <p>${article.title}</p>
                <button id="tab-${article.id}-close"><i class="bi bi-x"></i></button>
            </div>`;
...
let tabMouseDownStartPosition: {x: number, y: number} = {x: 0, y: 0};
let currentMovingTabId: number = -1;
let clickedTab: boolean = false;
let newTabPosition: number = 0;

const handleTabMouseDown = (event: any, id: number) => {
    tabMouseDownStartPosition = { x: event.clientX, y: event.clientY };
    currentMovingTabId = id;
    clickedTab = true;

    openTab(currentMovingTabId);
}
const handleTabMouseMove = (event: any) => {
    if (!clickedTab) {
        return;
    }
    if ((Math.abs(event.clientX - tabMouseDownStartPosition.x) < 10 &&
        Math.abs(event.clientY - tabMouseDownStartPosition.y) < 10) &&
        event.buttons == 0) {
...
const handleTabMouseUp = (event: any) => {
    if (!clickedTab) {
        return;
    }
...
document.body.onmousemove = (e) => handleTabMouseMove(e);
document.body.onmouseup = (e) => handleTabMouseUp(e);